...
/* DWT cycle counter fallback (used when interrupts are disabled, SysTick freezes) */
static uint8_t g_dwt_ready = 0;
static void delay_dwt_ensure_ready(void)
{
...
{
    if (__get_PRIMASK() & 1) {
        delay_dwt_ensure_ready();
        uint32_t sysclk_mhz = g_fac_us;
        uint32_t need = nus * sysclk_mhz;
